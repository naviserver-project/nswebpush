...
namespace eval webpush {
    ...
    test encrypt {} -body {
	set result [string map {"\n" {}} [ns_base64encode [encrypt -data "Push notification payload!" \
							       -privateKeyPem $::vapidCertPath/prime256v1_key.pem \
							       -auth [ns_base64urldecode 4LLU4S9l1S9IrPTsQZkPqw] \
							       -p256dh [ns_base64urldecode BNvOAPaPCCfpNCMR4AccTffxD8YMQbfIBWieLxgZE1qgU-YVVT5mOD7CaRRqg5ykA7_f8jm2VuOPZLvHn0moHas] \
	...
	append result [catch {[encrypt -data $tooLong \
				   -privateKeyPem $::vapidCertPath/prime256v1_key.pem \
				   -auth [ns_base64urldecode 4LLU4S9l1S9IrPTsQZkPqw] \
				   -p256dh [ns_base64urldecode BNvOAPaPCCfpNCMR4AccTffxD8YMQbfIBWieLxgZE1qgU-YVVT5mOD7CaRRqg5ykA7_f8jm2VuOPZLvHn0moHas] \
				   -salt [ns_base64decode WVGtEt/7tGKMNgqAeDvEPA==] \
